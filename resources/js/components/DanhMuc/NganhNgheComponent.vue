...
const consumer = {
  getListNganhNghe: function () {
    const url = "/api/nganh-nghe" + window.location.search;
    return axios.get(url).then((response) => response.data);
  },
  getNganhNghe: function (nn_id) {
    const url = "/api/nganh-nghe/" + nn_id;
    return axios.get(url).then((response) => response.data);
  },
  ...
  saveOrUpdate: function (formData) {
    if (formData.nn_id == null) {
      return axios.post("/api/nganh-nghe", formData);
    } else {
      return axios.put("/api/nganh-nghe/" + formData.nn_id, formData);
    }
  },
  destroy: function (nn_id) {
    return axios.delete("/api/nganh-nghe/" + nn_id);
  },
};
