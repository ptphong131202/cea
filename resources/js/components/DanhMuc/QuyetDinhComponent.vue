...
<script>
    ...
    const consumer = {
        getListQuyetDinh: function () {
            const url = '/api/quyet-dinh' + window.location.search;
            return axios.get(url).then(response => response.data);
        },
        getQuyetDinh: function (qd_id) {
            const url = '/api/quyet-dinh/' + qd_id;
            return axios.get(url).then(response => response.data);
        },
        checkUsed: function (qd_id) {
            const url = '/api/quyet-dinh/check-used/' + qd_id;
            return axios.get(url).then(response => response.data);
        },
        saveOrUpdate: function (formData) {
            if (formData.qd_id == null) {
                return axios.post('/api/quyet-dinh', formData);
            } else {
                return axios.put('/api/quyet-dinh/' + formData.qd_id, formData);
            }
        },
        destroy: function (qd_id) {
            return axios.delete('/api/quyet-dinh/' + qd_id);
        }
    }
