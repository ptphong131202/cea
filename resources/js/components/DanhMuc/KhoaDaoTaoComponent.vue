...
const consumer = {
  getListNganhNghe: function (hdt_id) {
    const url = "/api/nganh-nghe/all?hedaotao=" + hdt_id;
  ...
  getListKhoaDaoTao: function () {
    const url = "/api/khoa-dao-tao" + window.location.search;
    return axios.get(url).then((response) => response.data);
  },
  getKhoaDaotao: function (kdt_id) {
    const url = "/api/khoa-dao-tao/" + kdt_id;
    return axios.get(url).then((response) => response.data);
  },
  saveOrUpdate: function (formData) {
    if (formData.kdt_id == null) {
      return axios.post("/api/khoa-dao-tao", formData);
    } else {
      return axios.put("/api/khoa-dao-tao/" + formData.kdt_id, formData);
    }
  },
  destroy: function (kdt_id) {
    return axios.delete("/api/khoa-dao-tao/" + kdt_id);
  },
  duplicate: function (formData) {
    return axios.post("/api/khoa-dao-tao/duplicate/" + formData.kdt_id, formData);
  },
};
