...
<script>
    ...
    const consumer = {
        getListNienKhoa: function () {
            const url = '/api/nien-khoa' + window.location.search;
            return axios.get(url).then(response => response.data);
        },
        getNienKhoa: function (nk_id) {
            const url = '/api/nien-khoa/' + nk_id;
            return axios.get(url).then(response => response.data);
        },
        saveOrUpdate: function (formData) {
            if (formData.nk_id == null) {
                return axios.post('/api/nien-khoa', formData);
            } else {
                return axios.put('/api/nien-khoa/' + formData.nk_id, formData);
            }
        },
        destroy: function (nk_id) {
            return axios.delete('/api/nien-khoa/' + nk_id);
        }
    }
