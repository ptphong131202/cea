...
<script>
    ...
    const consumer = {
        getListHeDaoTao: function () {
            const url = '/api/he-dao-tao' + window.location.search;
            return axios.get(url)
                    .then(response => response.data);
        },
        getHeDaotao: function (hdt_id) {
            const url = '/api/he-dao-tao/' + hdt_id;
            return axios.get(url)
                    .then(response => response.data);
        },
        saveOrUpdate: function (formData) {
            if (formData.hdt_id == null) {
                return axios.post('/api/he-dao-tao', formData);
            } else {
                return axios.put('/api/he-dao-tao/' + formData.hdt_id, formData);
            }
        },
        destroy: function (hdt_id) {
            return axios.delete('/api/he-dao-tao/' + hdt_id);
        }
    }
