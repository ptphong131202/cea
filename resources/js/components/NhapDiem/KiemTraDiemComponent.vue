...
import axios from 'axios';
  const consumer = {
    updateConfigSV: function (status) {
        const url = `/api/cau-hinh/${status}/update-config-allow-info-hssv`;
    ...
    getKetQuaHocTap: function (lh_id, semester) {
      const url = `/api/nhap-diem/${lh_id}/ket-qua-hoc-tap/json?semester=${semester}`;
    ...
    uploadFile: function (file, sheet) {
      ...
      return axios.post('/api/excel/import-score-check', formData, headers).then(res => res.data);
    }
  }
